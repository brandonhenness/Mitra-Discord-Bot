...
Run your own Mitra installation to manage your own machines or host a separate
bot for your community. Create your own Discord application and token; the
public invite in the README installs the hosted Mitra bot.

Before exposing an existing installation to public Discord servers, configure
the [infrastructure allowlist](public-access.md) on every node. Only the setup
wizard's explicitly saved `[bot].guild_id` is trusted when no allowlist is set;
without either, infrastructure commands are disabled everywhere. Public server
administrators cannot enable infrastructure access through Discord roles.

The [public invite and permissions guide](discord-install.md) covers the hosted
application's invite settings. Self-hosted infrastructure features may also need
Manage Roles for subscriptions and Attach Files and Pin Messages for dashboards.
The current setup wizard requests infrastructure permissions; grant the to-do
channel/thread permissions from that guide too if your installation uses lists.
...
See [UPS history storage](ups-history.md). Maintainers can publish tested,
versioned release packages through [the release workflow](releases.md).
...
- Role-based access (`Mitra Admin` and `Mitra Alerts` by default).
...
- `[bot]`: authorized Discord server IDs, channel ID, IP polling interval, role names
...
- `infrastructure_guild_ids`: Discord server IDs authorized to access this installation's infrastructure
...
assignments. See [Cloudflare setup and examples](cloudflare.md).
...
4. Configure `[bot].infrastructure_guild_ids` with your Discord server IDs; see [access configuration](public-access.md).
5. Start the bot.
6. Use commands in an authorized server:
   - `/about`
   - `/ip status`
   - `/alerts subscribe`
   - `/ups status`
...
- The bot ensures the admin role exists in operator-authorized Discord servers on startup.
...
See [private network setup and limitations](private-peer-network.md) and
[uptime monitoring setup](peer-uptime-monitoring.md). Alerts are opt-in through
